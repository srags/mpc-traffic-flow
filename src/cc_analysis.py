...
STATIC_COLUMNS = {
    ...
    'uncontrolled_tts':        ('Uncontrolled TTT (veh-hrs)',  lambda r: f"{r['uncontrolled_tts']:.2f}"),
    'controlled_tts':          ('Controlled TTT (veh-hrs)',    lambda r: f"{r['controlled_tts']:.2f}"),
...
def plot_avg_delay_bar_chart(results, save_path=None, text_fontsize=20):
    """
    ...
    fig, ax = plt.subplots(figsize=(max(8, 1.2 * len(dates)), 6))
    plt.grid()
