...
def mpc_opt(
    ...
    if safety_temporal is not None:
        active_segs = control_zone if control_zone is not None else seg_range
        for m in active_segs:
    ...
    if safety_spatial is not None:
        active_segs = control_zone[1:] if control_zone is not None else seg_range[1:]
        for m in active_segs:
            for h in range(horizon_p):
